...
def create_loaders(
    ...
    val_loader = DataLoader(
        validset, batch_size=1, shuffle=False, num_workers=num_workers, pin_memory=False
    )
...
def train(
    ...
    for i, sample in tqdm(enumerate(train_loader), total=len(train_loader)):
        start = time.time()
        inputs = [sample[key].cuda().float() for key in input_types] # [[b, 3, w, h], [b, 3, w, h]]
...
def main():
    ...
    if args.dataset == "nyudv2" and not args.is_debug:
        args.train_list = "data/nyudv2/train.txt"
        args.val_list = "data/nyudv2/val.txt"
        args.num_classes = 40
    elif args.dataset == "nyudv2" and args.is_debug:
        args.train_list = "data/nyudv2/train_debug.txt"
        args.val_list = "data/nyudv2/val_debug.txt"
        args.num_classes = 40
    elif args.dataset == "sunrgbd" and not args.is_debug:
        args.train_list = "data/sun/train.txt"
        args.val_list = "data/sun/test.txt"
        args.num_classes = 37
    elif args.dataset == "sunrgbd" and args.is_debug:
        args.train_list = "data/sun/train_debug.txt"
        args.val_list = "data/sun/test_debug.txt"
        args.num_classes = 37
    ...
    segmenter = DDP(
        segmenter, device_ids=[gpu], output_device=0, find_unused_parameters=True
    )
    ...
    for task_idx in range(args.start_stage, args.num_stages):
        optimizer = PolyWarmupAdamW(
            # encoder,encoder-norm,decoder
            params=[
                {
                    "params": param_groups[0],  # encoder
                    "lr": lrs[task_idx],
                    "weight_decay": args.weight_decay,
                },
                {
                    "params": param_groups[1],  # norm
                    "lr": lrs[task_idx],
                    "weight_decay": 0.0,
                },
                {
                    "params": param_groups[2],  # decoder
                    "lr": lrs[task_idx] * 10,
                    "weight_decay": args.weight_decay,
                },
            ],
