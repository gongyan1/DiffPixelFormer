...
if __name__ == "__main__":
    # pretrained_weights = torch.load("pretrained/swin_large_patch4_window7_224_22k.pth")
    wetr = WeTr("swin_large_window12", num_classes=20).cuda()
    # wetr.get_param_groupsv()
    params = parameter_count(wetr)
    print(f"Total parameters: {sum(p.numel() for p in wetr.parameters())}")
    print(f"Trainable parameters: {sum(p.numel() for p in wetr.parameters() if p.requires_grad)}")
    
    # NYUDV2
    # dummy_input = torch.rand(2, 1, 3, 480, 640).cuda()
    # SUNRGB-D
    dummy_input = torch.rand(2, 1, 3, 530, 730).cuda()
    # wetr(dummy_input)

    flops = FlopCountAnalysis(wetr, dummy_input)
    print(f"FLOPs: {flops.total() / 1e9} GFLOPs")


# if __name__ == "__main__":
#     wetr = WeTr("mit_b3", num_classes=20).cuda()

#     # 基础参数统计
#     total_params = sum(p.numel() for p in wetr.parameters())
#     trainable_params = sum(p.numel() for p in wetr.parameters() if p.requires_grad)
#     print(f"[PyTorch] Total parameters: {total_params}")
#     print(f"[PyTorch] Trainable parameters: {trainable_params}")

#     dummy_input = torch.rand(2, 1, 3, 530, 730).cuda()  # 必须严格按你的要求

#     # thop 统计
#     macs, params = profile(wetr, inputs=(dummy_input, ), verbose=False)
#     print(f"[thop] MACs: {macs / 1e9:.2f} GMACs")
#     print(f"[thop] Params: {params / 1e6:.2f} M")

#     # ptflops 统计（需自定义 input_constructor）
#     def input_constructor(input_res):
#         # 这里的 input_res 是 (1, 3, 530, 730)，你可以根据模型调整
#         return (torch.rand(2, 1, 3, 530, 730).cuda(),)

#     flops, params = get_model_complexity_info(
#         wetr,
#         (1, 3, 530, 730),  # 这里只能传单个样本shape，但实际 ptflops 有时不支持多batch多维输入
#         input_constructor=input_constructor,
#         as_strings=True,
#         print_per_layer_stat=False,
#         verbose=False
#     )
#     print(f"[ptflops] FLOPs: {flops}")
